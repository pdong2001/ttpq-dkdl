...
const FOLDER_PATH = 'images/departments';
const COVER = 'cover.jpg';

const IMAGES_HD1 = _.map(
  new Array(8).fill(FOLDER_PATH),
  (path, i) => `${path}/HD1/image (${i + 1}).jpg`,
);
const IMAGES_HD2 = _.map(
  new Array(10).fill(FOLDER_PATH),
  (path, i) => `${path}/HD2/image (${i + 1}).jpg`,
);
const IMAGES_BD = _.map(
  new Array(10).fill(FOLDER_PATH),
  (path, i) => `${path}/BD/image (${i + 1}).jpg`,
);
const IMAGES_HD = _.map(
  new Array(9).fill(FOLDER_PATH),
  (path, i) => `${path}/HD/image (${i + 1}).jpg`,
);
const IMAGES_TK = _.map(
  new Array(44).fill(FOLDER_PATH),
  (path, i) => `${path}/TK/image (${i + 1}).jpg`,
);
const IMAGES_MT = _.map(
  new Array(18).fill(FOLDER_PATH),
  (path, i) => `${path}/MT/image (${i + 1}).jpg`,
);
    ...
    desc: (
      <>
        ...
        <Container textAlign={'justify'} py='0'>
          ...
          <br />
          🌻“ Phát, phát và phát” tất cả những gì tốt nhất cho các Phật tử về thăm viếng Chùa là
          nhiệm vụ chính của ban Hành Đường 2.
          <br />
          ☘️Bằng tất cả tình yêu thương và sự quan tâm, các bạn Hành Đường 2 không những trao đi
          từng hộp cơm mà trao nhau cả nụ cười đầy thân ái kèm theo đó là lời chúc tốt lành.
          <br />
    ...
    desc: (
      <>
        <Container textAlign={'justify'} py='0'>
          ...
          Tri khách sinh viên,
          <br />
          Ân cần phụng sự, chu đáo tận tâm."
          <br />
          🔊🔊🔊 Ting ting
          <br />
          😊 Nghe tiếng tin nhắn đến, tự nhiên lòng Mr.Phụng Sự vui đến lạ. Cậu khẽ cười.
          <br />
          🙇🏻‍️ Mr. Tuổi Trẻ thấy vậy bèn tò mò hỏi Mr. Phụng Sự : Cậu ơi, có chuyện gì mà cậu vui
          thế?
          <br />
          💁🏻‍️ Mr.Phụng Sự: Cậu biết tin gì chưa? Chưa đầy 14 ngày nữa là diễn ra ĐẠI LỄ VU LAN rồi
          đó.
          <br />
          Và BAN TRI KHÁCH đang tuyển thành viên rất rầm rộ cậu ạ. Háo hức chưa?
          <br />
          🤷🏻‍️ Mr. Tuổi Trẻ ngạc nhiên: Vậy ban Tri Khách sẽ làm gì? 😊
          <br />
          💁🏻‍️ Mr.Phụng Sự trả lời: À, nhiệm vụ của ban Tri Khách là đón tiếp Phật tử gần xa với
          khẩu hiểu “ Ân cần - Lịch sự - Chu đáo - Tận Tâm ”.
          <br />
          <br />
          😊 ÂN CẦN quan tâm, hướng dẫn tận tình
          <br />
          😊 LỊCH SỰ trong từng lời nói và hành động.
          <br />
          😊 CHU ĐÁO, TẬN TÂM trong mọi nhiệm vụ.
          <br />
          🙆🏻‍️ Mr. Tuổi Trẻ: Uhm...nghe ý nghĩa quá nhỉ!
          <br />
        </Container>
      </>
    ),
    ...
    desc: (
      <>
        <Container>
          ...
          Khắp nơi đều tươi xanh
          <br />
          Nên nghiêng mình, cúi xuống
          <br />
          Nhặt từng cọng rác xinh
          <br />
          Cùng về đây bạn ơi! <br />
          Nơi núi rừng xanh tươi
          <br />
          Cùng nhau phân loại rác
          <br />
          Vào ban Môi trường thôi <br />
          <br />
          🤔Bạn là người luôn trăn trở với hệ thống thùng phân loại rác và mong được phổ biến ở khắp
          nơi nơi?
          <br />
          Bạn luôn khao khát mang tới mọi người thông điệp "Respect your environment. Please! Put
          your litter in the bin"? <br />
          😍Vậy xin chúc mừng bạn đã "LỌT TOP" vào ban Môi trường. Nhanh tay đăng ký kẻo ngay hết
          hạn.
          <br />
          Nhưng đừng đăng ký một mình nhé, phải nhiều "mình" mới vui.
        </Container>
