...
const Timeline = () => {
  const mediaQuery = useBreakpointValue({ base: 'small', sm: 'large', md: 'large', lg: 'large', xl: 'large' })
  const days_schedule = [
    {
    ...
    },
    {
      ...
      timelines: [
        ...
        {
          time: '04h30', content: 'Chương trình Lễ Thành Đạo', color: 'red.300', sub_content: [
        ...
        { time: '08h30', content: 'Họp tổng kết lễ Thành Đạo', sub_content: ['Giao lưu Ban Điều Hành Đạo Tràng, Chúng Thanh Niên'] },
      ...
      <Box w='100%' textAlign='center'>
        ...
        <Grid templateColumns='repeat(8, 1fr)' boxShadow='2xl'>
          <GridItem h={{ base: '200px', sm: '100%' }} colSpan={{ base: 8, sm: 3 }}>
            {mediaQuery == 'large' && <VStack h='100%' bg='darkBlue.700'>
              {days_schedule.map((item, index) => (
                <Box onClick={() => handleChooseDay(index)} cursor='pointer'
                  w='100%' h='100px'
                  ...
                  py={2}
                >
                  <Text textTransform='uppercase' fontWeight='600'>{item.day.titleDay}</Text>
                  <Text fontSize='14px'>{item.day.titleMonth} {item.day.titleDate}, {item.day.titleYear}</Text>
                </Box>
              ))}
            </VStack>}
            {mediaQuery == 'small' && <VStack spacing={0} h='100%'>
              <Box borderTopRadius="2xl" w='100%' h='25%' bg='ttpq.500' color='white' display='flex' justifyContent='center' alignItems='center'>
                {days_schedule[current_day].day.titleDay}
              </Box>
              <Box w='100%' h='70%' bg='white' display='flex' flexDirection='column'
                justifyContent='center'
                alignItems='center'>
                <Grid w='100%' templateColumns='repeat(8, 1fr)'>
                  <GridItem onClick={handlePrev} colSpan={1} cursor={current_day == 0 ? 'inherit' : 'pointer'}
                    display='flex' justifyContent='center' alignItems='center'>
                    <MdArrowBackIosNew opacity={current_day == 0 ? 0 : 1} />
                  </GridItem>
                  <GridItem colSpan={6}>
                    <Text fontSize="1.5em" color="gray">{days_schedule[current_day].day.titleMonth}</Text>
                    <Text fontSize="3em" color="black">{days_schedule[current_day].day.titleDate}</Text>
                  </GridItem>
                  <GridItem onClick={handleNext} colSpan={1} cursor={current_day == days_schedule.length - 1 ? 'inherit' : 'pointer'}
                    display='flex' justifyContent='center' alignItems='center'>
                    <MdArrowForwardIos opacity={current_day == days_schedule.length - 1 ? 0 : 1} />
                  </GridItem>
                </Grid>
              </Box>
              <Box w='100%' h='25%' bg='ttpq.500' color='white' display='flex' justifyContent='center' alignItems='center'>
            ...
            <Accordion defaultIndex={[0]}>
              {days_schedule[current_day].timelines.map((item, index) => (
                <AccordionItem textAlign='left'
                  ...
                  {
                    item.sub_content && <AccordionPanel pb={4}>
                      <UnorderedList>
